...
test('must raise when user does not exists', () => {
  ...
  expect(async () => await usecase.execute(input)).rejects.toThrow(new Error('User does not exists'));
});
...
test('must raise when pass wrong password', () => {
  ...
  expect(async () => await usecase.execute(input)).rejects.toThrow(new Error('Wrong password'));
});
...
test('must raise when account is not activated', () => {
  ...
  expect(async () => await usecase.execute(input)).rejects.toThrow(new Error('Inactive account'));
});
